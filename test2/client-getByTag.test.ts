...
it("supports default params", async ({ expect, client, docs }) => {
	...
	await client.getByTag(docs.default.tags[0], { routes: [] })
	expect(client).toHaveLastFetchedContentAPI({ lang: "fr-fr", routes: "[]" })
})
...
it("throws if the maximum number of retries with invalid refs is reached", async ({
	...
	await expect(() => client.getByTag(docs.default.tags[0])).rejects.toThrow(
		RefNotFoundError,
	)
...
it("supports signal", async ({ expect, client, docs }) => {
	await expect(() =>
		client.getByTag(docs.default.tags[0], {
...
it("shares concurrent equivalent network requests", async ({
	...
	await Promise.all([
		client.getByTag(docs.default.tags[0]),
		client.getByTag(docs.default.tags[0]),
		client.getByTag(docs.default.tags[0], { signal: controller1.signal }),
		client.getByTag(docs.default.tags[0], { signal: controller1.signal }),
		client.getByTag(docs.default.tags[0], { signal: controller2.signal }),
		client.getByTag(docs.default.tags[0], { signal: controller2.signal }),
	])
	await client.getByTag(docs.default.tags[0])
