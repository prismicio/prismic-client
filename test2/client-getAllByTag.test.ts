...
it("supports default params", async ({ expect, client, docs }) => {
	...
	await client.getAllByTag(docs.default.tags[0], { routes: [] })
	expect(client).toHaveLastFetchedContentAPI({ lang: "fr-fr", routes: "[]" })
})
...
it("throws if the maximum number of retries with invalid refs is reached", async ({
	...
	await expect(() => client.getAllByTag(docs.default.tags[0])).rejects.toThrow(
		RefNotFoundError,
	)
...
it("supports signal", async ({ expect, client, docs }) => {
	await expect(() =>
		client.getAllByTag(docs.default.tags[0], {
...
it("shares concurrent equivalent network requests", async ({
	...
	await Promise.all([
		client.getAllByTag(docs.default.tags[0]),
		client.getAllByTag(docs.default.tags[0]),
		client.getAllByTag(docs.default.tags[0], { signal: controller1.signal }),
		client.getAllByTag(docs.default.tags[0], { signal: controller1.signal }),
		client.getAllByTag(docs.default.tags[0], { signal: controller2.signal }),
		client.getAllByTag(docs.default.tags[0], { signal: controller2.signal }),
	])
	await client.getAllByTag(docs.default.tags[0])
