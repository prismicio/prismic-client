...
it("uses preview ref from browser cookie", async ({ expect, client, masterRef, response }) => {
...
it("re-enables after being disabled", async ({ expect, client, masterRef, response }) => {
	vi.stubGlobal("document", { cookie: `io.prismic.preview=foo` })
