...
it("throws if no document is returned", async ({ expect, client, docs, response }) => {
	vi.mocked(client.fetchFn).mockImplementationOnce(fetch).mockResolvedValueOnce(response.search([]))
	await expect(() => client.getByUID(docs.default.type, "invalid")).rejects.toThrow(NotFoundError)
})
...
it("includes filter", async ({ expect, client, docs }) => {
	...
	params.append("q", `[[at(my.${docs.default.type}.uid, "${docs.default.uid}")]]`)
