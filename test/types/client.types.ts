...
// With generic
const genericGetAllByIDs = await defaultClient.getAllByIDs<FooDocument>([
	"qux",
]);
...
expectType<TypeEqual<typeof genericGetByUIDs, prismic.Query<FooDocument>>>(
	true,
);
...
// Documents (known)
const documentsGetAllByUIDs = await documentsClient.getAllByUIDs("foo", [
	"qux",
]);
...
expectType<TypeEqual<typeof genericGetByType, prismic.Query<FooDocument>>>(
	true,
);
