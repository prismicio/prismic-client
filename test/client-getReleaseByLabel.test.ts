...
it("returns release with matching label", async ({ expect, client, accessToken, release }) => {
	client.accessToken = accessToken
	const res = await client.getReleaseByLabel(release.label)
	expect(res).toMatchObject({ ref: release.ref })
})

it("throws if release with label is not found", async ({ expect, accessToken, client }) => {
	client.accessToken = accessToken
	await expect(() => client.getReleaseByLabel("invalid")).rejects.toThrow(PrismicError)
})
