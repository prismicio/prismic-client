...
it("supports an explicit access token", async ({ expect, client, accessToken }) => {
...
it("throws ForbiddenError on 401", async ({ expect, client }) => {
	vi.mocked(client.fetchFn).mockResolvedValue(Response.json({}, { status: 401 }))
...
it("throws RepositoryNotFoundError on 404", async ({ expect, client }) => {
	vi.mocked(client.fetchFn).mockResolvedValue(Response.json({}, { status: 404 }))
	await expect(() => client.getRepository()).rejects.toThrow(RepositoryNotFoundError)
})
