...
it("returns paginated response", async ({ expect, client, docs }) => {
	const res = await client.getByUIDs(docs.default.type, [docs.default.uid, docs.default2.uid])
	expect(res).toMatchObject({ results: expect.any(Array) })
})

it("includes filter", async ({ expect, client, docs }) => {
	await client.getByUIDs(docs.default.type, [docs.default.uid, docs.default2.uid])
